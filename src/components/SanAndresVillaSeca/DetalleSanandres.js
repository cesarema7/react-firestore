...
class DetalleSanandres extends Component {
  ...
  delete(id){
    firebase.firestore().collection('solicitudes-sanandres').doc(id).delete().then(() => {
  ...
  aprobar(id){
    firebase.firestore().collection('solicitudes-sanandres').doc(id).update({estadosoli: 'APROBADO'}).then(() => {
  ...
  rechazar(id){
    firebase.firestore().collection('solicitudes-sanandres').doc(id).update({estadosoli: 'RECHAZADO'}).then(() => {
      console.log("Document successfully actualizado!");
      this.props.history.push("/lista-solicitudes-pendientes")
    }).catch((error) => {
      console.error("Error actualizando document: ", error);
        ...
        <div align="start" class="panel panel-default" class="col-md-5">
          <div class="panel-heading">
            <div className="mt-3">
              ...
              <h4><Link id="btn-lsr" to="/lista-solicitudes-san-andres-villa-seca" class="btn btn-primary">Listado de solicitudes SAN ANDRÉS VILLA SECA<ListIcon/> </Link></h4>
              <h4><Link id="btn-lsa" to="/lista-solicitudes-aprobadas" class="btn btn-primary">Listado de solicitudes<ListIcon/> </Link></h4>
            </div>
          
            <h3 class="panel-title">
              {this.state.solicitudessanandres.estadosoli}
            </h3>
          </div>
          <div class="panel-body">
            <dl>
              <dt>Distrito:</dt>
              <dd>{this.state.solicitudessanandres.distrito}</dd>
              <dt>Destino:</dt>
              <dd>{this.state.solicitudessanandres.destino}</dd>
              <dt>Vehículo:</dt>
              <dd>{this.state.solicitudessanandres.vehiculo}</dd>
              <dt>Placa del vehículo:</dt>
              <dd>{this.state.solicitudessanandres.placa}</dd>
              <dt>Personas que conforman la comision:</dt>
              <dd>{this.state.solicitudessanandres.personas}</dd>
              <dt>Cantidad de Combustible que Solicita en Quetzales:</dt>
              <dd>{this.state.solicitudessanandres.cantidad}</dd>
              <dt>Detalle de comisión:</dt>
              <dd>{this.state.solicitudessanandres.detalle}</dd>
              <dt>Hora y Fecha de salida:</dt>
              <dd>{this.state.solicitudessanandres.fechaS}</dd>
              <dt>Fecha de regreso:</dt>
              <dd>{this.state.solicitudessanandres.fechaR}</dd>
              <dt>Piloto:</dt>
              <dd>{this.state.solicitudessanandres.piloto}</dd>
              <dt>Vo.Bo:</dt>
              <dd>{this.state.solicitudessanandres.autorizada}</dd>
            </dl>
            <Link id="btn-editar" to={`/editar-solicitud-san-andres-villa-seca/${this.state.key}`} class="btn btn-warning">Editar <EIcon /> </Link>&nbsp;
...
export default DetalleSanandres;