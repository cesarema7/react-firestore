...
class EditarChamperico extends Component {
  ...
  componentDidMount() {
    ...
    ref.get().then((doc) => {
      if (doc.exists) {
        const solicitudchamperico = doc.data();
        this.setState({
          key: doc.id,
          estadosoli: solicitudchamperico.estadosoli,
          distrito: solicitudchamperico.distrito,
          cantidad: solicitudchamperico.cantidad,
          destino: solicitudchamperico.destino,
          fechaS: solicitudchamperico.fechaS,
          personas: solicitudchamperico.personas,
          piloto: solicitudchamperico.piloto,
          vehiculo: solicitudchamperico.vehiculo,
          placa: solicitudchamperico.placa,
          detalle: solicitudchamperico.detalle,
          fechaR: solicitudchamperico.fechaR,
          autorizada: solicitudchamperico.autorizada
        });
      } else {
  ...
  onChange = (e) => {
    ...
    this.setState({solicitudchamperico:state});
  }
  ...
  onSubmit = (e) => {
    ...
    const updateRef = firebase.firestore().collection('solicitudes-champerico').doc(this.state.key);
    updateRef.set({
      estadosoli,
      distrito,
      cantidad,
        ...
        <div className="m-5">
          <Grid container spacing={3}>
            <Grid className="mx-auto">
              <div>
              <h4><Link to={`/detalle-solicitud-champerico/${this.state.key}`} class="btn btn-primary">Volver a detalles de solicitud CHAMPERICO</Link></h4>
			  </div>
			  <div>
...
export default EditarChamperico;