...
class DetalleCblanco extends Component {
  ...
  componentDidMount() {
    const ref = firebase.firestore().collection('solicitudes-cblanco').doc(this.props.match.params.id);
    ref.get().then((doc) => {
      if (doc.exists) {
        this.setState({
          solicitudcblanco: doc.data(),
          key: doc.id,
          isLoading: false
        });
      } else {
  ...
  delete(id){
    firebase.firestore().collection('solicitudes-cblanco').doc(id).delete().then(() => {
  ...
  aprobar(id){
    firebase.firestore().collection('solicitudes-cblanco').doc(id).update({estadosoli: 'APROBADO'}).then(() => {
  ...
  rechazar(id){
    firebase.firestore().collection('solicitudes-cblanco').doc(id).update({estadosoli: 'RECHAZADO'}).then(() => {
      console.log("Document successfully actualizado!");
      this.props.history.push("/lista-solicitudes-pendientes")
    }).catch((error) => {
      console.error("Error actualizando document: ", error);
        ...
        <div align="start" class="panel panel-default" class="col-md-5">
          <div class="panel-heading">
            <div className="mt-3">
              ...
              <h4><Link id="btn-lsr" to="/lista-solicitudes-caballo-blanco" class="btn btn-primary">Listado de solicitudes CABALLO BLANCO<ListIcon/> </Link></h4>
              <h4><Link id="btn-lsa" to="/lista-solicitudes-aprobadas" class="btn btn-primary">Listado de solicitudes<ListIcon/> </Link></h4>
            </div>
          
            <h3 class="panel-title">
              {this.state.solicitudcblanco.estadosoli}
            </h3>
          </div>
          <div class="panel-body">
            <dl>
              <dt>Distrito:</dt>
              <dd>{this.state.solicitudcblanco.distrito}</dd>
              <dt>Destino:</dt>
              <dd>{this.state.solicitudcblanco.destino}</dd>
              <dt>Vehículo:</dt>
              <dd>{this.state.solicitudcblanco.vehiculo}</dd>
              <dt>Placa del vehículo:</dt>
              <dd>{this.state.solicitudcblanco.placa}</dd>
              <dt>Personas que conforman la comision:</dt>
              <dd>{this.state.solicitudcblanco.personas}</dd>
              <dt>Cantidad de Combustible que Solicita en Quetzales:</dt>
              <dd>{this.state.solicitudcblanco.cantidad}</dd>
              <dt>Detalle de comisión:</dt>
              <dd>{this.state.solicitudcblanco.detalle}</dd>
              <dt>Hora y Fecha de salida:</dt>
              <dd>{this.state.solicitudcblanco.fechaS}</dd>
              <dt>Fecha de regreso:</dt>
              <dd>{this.state.solicitudcblanco.fechaR}</dd>
              <dt>Piloto:</dt>
              <dd>{this.state.solicitudcblanco.piloto}</dd>
              <dt>Vo.Bo:</dt>
              <dd>{this.state.solicitudcblanco.autorizada}</dd>
            </dl>
            <Link id="btn-editar" to={`/editar-solicitud-caballo-blanco/${this.state.key}`} class="btn btn-warning">Editar <EIcon /> </Link>&nbsp;
