...
class EditarSanse extends Component {
  ...
  componentDidMount() {
    ...
    ref.get().then((doc) => {
      if (doc.exists) {
        const solicitudsanse = doc.data();
        this.setState({
          key: doc.id,
          estadosoli: solicitudsanse.estadosoli,
          distrito: solicitudsanse.distrito,
          cantidad: solicitudsanse.cantidad,
          destino: solicitudsanse.destino,
          fechaS: solicitudsanse.fechaS,
          personas: solicitudsanse.personas,
          piloto: solicitudsanse.piloto,
          vehiculo: solicitudsanse.vehiculo,
          placa: solicitudsanse.placa,
          detalle: solicitudsanse.detalle,
          fechaR: solicitudsanse.fechaR,
          autorizada: solicitudsanse.autorizada
        });
      } else {
  ...
  onChange = (e) => {
    ...
    this.setState({solicitudsanse:state});
  }
  ...
  onSubmit = (e) => {
    ...
    const updateRef = firebase.firestore().collection('solicitudes-sanse').doc(this.state.key);
    updateRef.set({
      estadosoli,
      distrito,
      cantidad,
        ...
        <div className="m-5">
          <Grid container spacing={3}>
            <Grid className="mx-auto">
              <div>
              <h4><Link to={`/detalle-solicitud-san-sebastian/${this.state.key}`} class="btn btn-primary">Volver a detalles de solicitud SAN SEBASTIÁN</Link></h4>
			  </div>
			  <div>
